...
async def create_appointment(appt_request: schemas.AppointmentCreate, db: Session = Depends(get_db)):
   
    db_appt = AppointmentRepo.fetch_by_name(db, name=appt_request.patient_name)
    print(db_appt)
    if db_appt:
        raise HTTPException(status_code=400, detail="Appointment already exists!")

    return await AppointmentRepo.create(db=db, appt=appt_request)



@app.get('/appointments/{doctor_id}', tags=["Appointment"], response_model=List[schemas.Appointment])
def get_all_appointments(doctor_id: Optional[str] = None,db: Session = Depends(get_db)):

    if doctor_id:
        appointments = []
        db_store = AppointmentRepo.fetch_by_name(db, doctor_id)
        print(db_store)
